...
// --- MODIFIKASI: Tambahan EEPROM.commit() khusus untuk ESP32 ---
#if defined(ESP32)
...
{
// --- MODIFIKASI: Inisialisasi EEPROM untuk ESP32 ---
#if defined(ESP32)
    EEPROM.begin(32); // Alokasi 32 byte (cukup untuk menyimpan nilai K)
#endif
    // --------------------------------------------------

    EEPROM_read(KVALUEADDR, this->_kvalueLow); // read the calibrated K value from EEPROM

    // Cek apakah EEPROM masih kosong (0xFF) atau NaN
    if (isnan(this->_kvalueLow) || (EEPROM.read(KVALUEADDR) == 0xFF && EEPROM.read(KVALUEADDR + 1) == 0xFF && EEPROM.read(KVALUEADDR + 2) == 0xFF && EEPROM.read(KVALUEADDR + 3) == 0xFF))
    {
    ...
    if (isnan(this->_kvalueHigh) || (EEPROM.read(KVALUEADDR + 4) == 0xFF && EEPROM.read(KVALUEADDR + 5) == 0xFF && EEPROM.read(KVALUEADDR + 6) == 0xFF && EEPROM.read(KVALUEADDR + 7) == 0xFF))
    {
...
float DFRobot_EC::readEC(float voltage, float temperature)
{
    ...
    // --- START MODIFIKASI SKRIPSI (SINGLE POINT) ---
    // Sensor akan selalu menggunakan kalibrasi dari 1.413 (_kvalueLow) berapapun tingginya EC.
    // Jika ingin kembali ke kode asli, hapus komentar (//) pada KODE ASLI di bawah.
    this->_kvalue = this->_kvalueLow;

    // KODE ASLI (Hapus awalan // untuk mengaktifkan):
    // if (valueTemp > 2.5) {
