...
// Kita samakan size-nya dengan library EC (32 byte) agar konsisten
#if defined(ESP32)
    EEPROM.begin(32);
#endif
    // --------------------------------------------------
    ...
    // Cek validitas data EEPROM (ESP32 seringkali return NaN atau 0 jika belum diinit)
    if (isnan(this->_neutralVoltage) || (EEPROM.read(PHVALUEADDR) == 0xFF && EEPROM.read(PHVALUEADDR + 1) == 0xFF && EEPROM.read(PHVALUEADDR + 2) == 0xFF && EEPROM.read(PHVALUEADDR + 3) == 0xFF))
    {
    ...
    if (isnan(this->_acidVoltage) || (EEPROM.read(PHVALUEADDR + 4) == 0xFF && EEPROM.read(PHVALUEADDR + 5) == 0xFF && EEPROM.read(PHVALUEADDR + 6) == 0xFF && EEPROM.read(PHVALUEADDR + 7) == 0xFF))
    {
