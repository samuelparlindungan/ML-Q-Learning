...
def inisialisasi_csv(): 
    if not os.path.exists(CSV_FILENAME): 
        df = pd.DataFrame(columns=[ 
            ...
            "pH_St+1", "EC_St+1", "Delta_pH", "Delta_EC", "Timestamp" 
        ]) 
...
if __name__ == "__main__": 
    ...
    try:
        while True:
            ...
            print(f">>> [LOCKED St] pH: {ph_st} | EC: {ec_st} | Sampel: {len(buffer_ph_st)} data <<<")
            ...
            print(f"Hardware selesai! Menunggu homogenisasi {WAKTU_HOMOGENISASI} detik (3 Menit)...")
            ...
            if len(buffer_ph_st1) == 0:
                print("PERINGATAN: Tidak ada data sensor St+1 masuk! Data tidak disimpan.")
                continue

            ph_st1 = round(sum(buffer_ph_st1) / len(buffer_ph_st1), 2)
            ec_st1 = round(sum(buffer_ec_st1) / len(buffer_ec_st1), 2)
            
            print(f">>> [LOCKED St+1] pH: {ph_st1} | EC: {ec_st1} | Sampel: {len(buffer_ph_st1)} data <<<")
            ...
            df_baru.to_csv(CSV_FILENAME, mode='a', header=not os.path.exists(CSV_FILENAME), index=False)
